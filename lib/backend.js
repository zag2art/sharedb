...
Backend.prototype.subscribe = function(agent, index, id, version, callback) {
  ...
  backend.pubsub.subscribe(channel, function(err, stream) {
    ...
    if (version == null) {
      ...
      backend.getOps(collection, index, id, version, null, function(err, ops) {
        if (err) return callback(err);
...
Backend.prototype.queryResubscribe = function(agent, index, query, emitter, options, callback) {
  var start = Date.now();
  var backend = this;
  backend._triggerQuery(agent, index, query, options, function(err, request) {
    if (err) return callback(err);
    ...
    emitter.query = request.query;
    emitter.canPollDoc = emitter.db.canPollDoc(emitter.collection, emitter.query);

    emitter.queryPoll();

    backend.emit('timing', 'queryResubscribe', Date.now() - start, request);
    callback();
  });
};
