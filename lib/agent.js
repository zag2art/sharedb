...
Agent.prototype._subscribeToQuery = function(emitter, queryId, index, query) {
  ...
  emitter.onError = function(err) {
    ...
    // may not be the client's fault, and they were not the result of a
    // direct request by the client
    console.error('Query subscription stream error', index, query, err);
  };

  emitter.onOp = function(op) {
    var id = op.d;
    if (agent._isOwnOp(index, op)) return;
    agent._sendOp(index, id, op);
  };
};
