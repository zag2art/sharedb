...
Agent.prototype._sendOp = function(collection, id, op) {
  ...
  var projection = backend.projections[collection];

  if (projection) {
    try {
      projections.projectOp(projection.fields, op);
    } catch (err) {
      console.error('Can not project ops', index, query);
      return;
    }
  }
...
Agent.prototype._checkRequest = function(request) {
  if (request.a === 'qf' || request.a === 'qs' || request.a === 'qu' || request.a === 'qr') {
...
Agent.prototype._handleMessage = function(request, callback) {
  try {
    ...
    switch (request.a) {
      ...
      case 'qr':
        return this._queryResubscribe(request.id, request.c, request.q, getQueryOptions(request), callback);
      case 'qu':
...
Agent.prototype._queryResubscribe = function(queryId, collection, query, options, callback) {
